...
hitSon = pygame.mixer.Sound(chemin_repertoire + '\contenu\son\hit.mp3')
...
def salle_mort(x_ph, y_ph, fond):
    global ph_descendre, ph_monter
    fenetre.blit(fond, (0, 0))
    if ph_descendre == False:
        fenetre.blit(phantome_mort, (x_ph, y_ph-2))
        if y_ph < 300:
            ph_descendre = True
    else:
        fenetre.blit(phantome_mort, (x_ph, y_ph+2))
...
def jeu():
    global touche, nbr_fond, fond
    run = True
    while run:
        if nbr_fond == "mort":
            salle_mort(600, 300, fond)
        fps.tick(30)
        drawFenetre()
