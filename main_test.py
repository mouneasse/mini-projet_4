...
def drawFenetre(): # fonction qui dessine les niveaux et leur éléments
    global anim_cooldown, tempsInitial, change_fond, fond, nbr_fond, nbr_e, draw_pnj, zone_pnj, anim_parchemin, nbr_dialogue, deja_change, trouve_arg, zone_arg, go_bateau, x_pnj, sortie, ph_descendre, y_ph, x_ph, random, zone_tapis
    ...
    if change_fond == True:  # on change l'image du niveau
        ...
        if nbr_fond == 0:
            ...
            change_fond = False
            pygame.mixer.Channel(6).play(pygame.mixer.Sound(chemin_repertoire + "\contenu\son\\boss_mort.mp3"))
    ...
    if nbr_fond != "mort": # on dessine tout le temps sauf pour la salle mort les éléments en dessous
        ...
        for e in liste_ennemi:
            e.drawennemi()
    if zone_tapis == True and nbr_fond != 4: # on test si le joueur est a gauche pour le passage des salles
...
class joueur(object):  # class qui contient tout le éléments inérants à au joueur
    ...
    def mouvement(self,touche): # fonction qui permet le mouvement du joueur (ses déplacements ainsi que ses collisions)
        global nbr_fond, change_fond, list_nbr_e, zone_pnj, inter_pnj, deja_change, sortie, sortie_possible, trouve_arg, nbr_dialogue, anim_parchemin, zone_arg, go_bateau, zone_tapis
        zone_arg = False
        x_j_orig = self.x_j
        y_j_orig = self.y_j
...
class boss(object):  # class qui contient tout le éléments inérants à au boss
    ...
    def drawboss(self): # permet de dessiner le boss
        ...
        pygame.draw.rect(fenetre, VERT, (fenetre_largeur//2 - 500, fenetre_hauteur - 80, 1000 - ((1000 / 35) * (35 - self.vie)), 70))
