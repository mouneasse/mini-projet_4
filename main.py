...
def salle_acceuille():
    ...
    if commencer <= 10:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if (410 < position[0] < 760) and (180 < position[1] < 390):  # collision des caisses au milieu de la salle
                if 390 <= position[0] <= 430:  # coté droit
                    collision_droite = True
                    position = position.move(-velocite_joueur, 0)
                elif 740 <= position[0] <= 780:  # coté gauche
                    collision_gauche = True
                    position = position.move(velocite_joueur, 0)
                elif 160 <= position[1] <= 200:  # coté bas
                    collision_basse = True
                    position = position.move(0, -velocite_joueur)
                elif 370 <= position[1] <= 410:  # coté haut
                    collision_haute = True
                    position = position.move(0, velocite_joueur)

            elif position[0] <= -30:  # collision du mur de gauche
                collision_gauche = True
                position = position.move(velocite_joueur, 0)
            elif position[0] >= 1200:  # # collision du mur de droite
                collision_droite = True
                position = position.move(-velocite_joueur, 0)
            elif position[1] <= -105:  # collision du mur du haut
                collision_haute = True
                position = position.move(0, velocite_joueur)
            ...
            if (position[0] <= 135) and (165 <= position[1] <= 375):
                tapis_rouge_salle_acceuille = True

            elif (position[0] >= 1065) and (165 <= position[1] <= 375):
...
def niveau1():
    global heros, commencer, position, salle_niveau1, collision_gauche, collision_droite, collision_haute, collision_basse
    ...
    if commencer <= 100:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if position[0] <= -50:  # collision du mur de gauche
                collision_gauche = True
                position = position.move(velocite_joueur, 0)
            elif position[0] >= 1250:  # # collision du mur de droite
                collision_droite = True
                position = position.move(-velocite_joueur, 0)
            elif position[1] <= -30:  # collision du mur du haut
                collision_haute = True
                position = position.move(0, velocite_joueur)
            elif position[1] >= 585:  # collision du mur du bas
                collision_basse = True
                position = position.move(0, -velocite_joueur)
            else:  # si il n'y a pas de collision alors on met les variable de collision à 0
                collision_haute = False
